...
from app.crud.product_crud import (
    add_new_product,
    get_all_products as fetch_all_products_from_db,  # Rename the import to avoid conflicts
    get_product_by_id,
...
# Updated function name to avoid recursion issue
@app.get("/manage-products/all", response_model=list[Product])
def fetch_all_products(session: Annotated[Session, Depends(get_session)]):
    """ Get all products from the database """
    return fetch_all_products_from_db(session)  # Use the renamed imported function
